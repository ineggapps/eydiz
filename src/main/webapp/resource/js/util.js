...
function isValidResidentNO(ssn1, ssn2) {
  ...
  for (i = 0; i < 13; i++) {
    if (i < 6) ssn[i] = parseInt(ssn1.charAt(i));
    else ssn[i] = parseInt(ssn2.charAt(i - 6));
  }

  temp = ssn1.substr(0, 2);
  if (temp.charAt(0) == "0") temp = temp.charAt(1);
  year = parseInt(temp);

  if (ssn[6] == 1 || ssn[6] == 2 || ssn[6] == 5 || ssn[6] == 6) year = year + 1900;
  else year = year + 2000;
  ...
  if (ssn[6] < 1 || ssn[6] > 4) return false;
  ...
  if (day > days[month - 1] || day < 1) return false;

  tot = 0;
  for (i = 0; i < 12; i++) tot = tot + ssn[i] * check[i];
  chkNum = 11 - (tot % 11);
  chkNum = chkNum % 10;

  if (chkNum != ssn[12]) return false;
  return true;
}
...
function getElementsByAttr(attrName, attrValue) {
  ...
  for (var i = 0; i < elements.length; i++) {
    if (
      elements[i].hasAttribute(attrName) &&
...
function isValidSpecialChar(str) {
  if (str.value.search(/[\",\',<,>]/g) >= 0) return true; // 존재 하면
  return false;
}
